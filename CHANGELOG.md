...
- 📑 **Documentazione progetto**: cartella `docs/` con `business-brief`, `studio-fattibilita`, `roadmap`, `deploy`, `aggiungere-un-marchio`
- 📜 `CHANGELOG.md` + `LICENSE.txt` (licenza proprietaria)
...
- `.gitignore`: aggiunti `.claude/`, `bazzana-sito/`, `__pycache__/`, `*.pyc`
- `sitemap.xml`: 6 nuove entry per le pagine Weibang
